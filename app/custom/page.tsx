...
export default function Custom() {
  ...
  const simulateWFR= () => {
    ...
    references.forEach((ref, stepIndex) => {
      const step: Step = {
        reference: ref,
        frames: [...frames], 
        status: 'Hit'
      ...
      frequencyMap.set(ref, (frequencyMap.get(ref) || 0) + 1);
      lastUsed.set(ref, stepIndex);
      ...
      if (!frames.includes(ref)) {
        ...
        if (frames.includes('-')) {
          ...
          frames.forEach((frame, index) => {
            const frequency = frequencyMap.get(frame) || 1; 
            const age = stepIndex - (lastUsed.get(frame) || 0);
            const priority = (0.7 * (frequency)) + (0.3 * age);
        ...
        {steps.length > 0 && (
          <>
            ...
            <Graph
              ...
              title="WFR Page Faults Progression"
              lineColor="#8b5cf6"
              pointColor="#ec4899"
            />
          </>
