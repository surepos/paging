...
export default function FIFO() {
  ...
  const simulateFIFO = () => {
    ...
    const frames: string[] = Array(frameCount).fill('-'); // Initialize with placeholders
    const queue: string[] = [];
    ...
    references.forEach((ref) => {
      ...
      if (!frames.includes(ref)) {
        ...
        if (queue.length < frameCount) {
          frames[queue.length] = ref;
          queue.push(ref);
        } else {
        
          const oldest = queue.shift()!;
          const oldestIndex = frames.indexOf(oldest);
      
          for (let i = oldestIndex; i < frames.length - 1; i++) {
            frames[i] = frames[i + 1];
          }
          frames[frames.length - 1] = ref;
          queue.push(ref);
        }
        step.frames = [...frames];
      }
        ...
        {steps.length > 0 && (
          <>
            ...
            <Graph
              ...
              title="FIFO Page Faults Progression"
              lineColor="#8b5cf6"
              pointColor="#ec4899"
            />
          </>
